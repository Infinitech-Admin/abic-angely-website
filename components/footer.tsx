          ...
          <div className="flex space-x-16">
            <div className="footer-col">
              ...
              <ul className="text-base">
                {["Home", "About Us", "What’s New", "Properties", "Services", "Careers", "Contact Us"].map(
                  (link, index) => (
                    <li key={index}>
                      <a className="flex items-center hover:text-violet-400" href={`/${link.toLowerCase().replace(" ", "")}`}>
                        <IoMdArrowDropright size={18} /> {link}
                      </a>
                    </li>
                  )
                )}
            ...
            <div className="footer-col max-w-xs">
              ...
              <ul className="links">
                {[
                  ...
                  { name: "Room Planner", path: "/" },
                ].map((feature, index) => (
                  <li key={index}>
                    <a className="flex items-center hover:text-violet-400" href={feature.path}>
          ...
          <div className="footer-col md:max-w-md">
            ...
            <form onSubmit={handleSubmit} className="py-4">
              <div className="flex items-center gap-4">
                <Input
                  ...
                  disabled={loading}
                />
                <Button type="submit" className="bg-violet-200 text-violet-900" size="lg">
            ...
            <div className="flex gap-2">
              {[  
                ...
                { icon: <FaTelegram size={18} />, url: `https://t.me/+63${socialLinks.telegram}` },
                { icon: <FaViber size={18} />, url: `viber://chat?number=${socialLinks.viber}` },
                { icon: <FaWhatsapp size={18} />, url: `https://wa.me/${socialLinks.whatsapp}` }
              ].map((item, index) => (
                <a 
                  key={index} 
                  href={item.url || "#"} 
                  ...
                  className={`px-2 py-2 rounded-lg ${
                    item.url ? "bg-violet-300 text-violet-700" : "bg-gray-500 text-gray-300 cursor-not-allowed"
                  }`}
                >
                  {item.icon}
                </a>
      ...
      <div className="bg-violet-800 text-center py-4 px-12">
        <div className="flex flex-col md:flex-row justify-center items-center text-center md:gap-4">
          ...
          <div className="flex flex-col md:flex-row items-center md:gap-4 text-sm">
            <Link href="/termsandconditions" aria-label="Terms and Conditions">Terms and Conditions</Link>
            <Link href="/dataprivacy" aria-label="Data and Privacy">Data and Privacy</Link>
          </div>
        </div>
      </div>
    </footer>
