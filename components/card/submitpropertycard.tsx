                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    {previews.map((src, index) => (
                      <div key={index} className="relative">
                        <img src={src} alt={`Preview ${index + 1}`} className="rounded-lg h-24" />
                        <button className="absolute z-50 top-0 right-0 bg-red-500 text-white text-xs rounded-full px-1 py-1" onClick={() => removeImage(index)}>
                          <LuX size={18}/>
                        </button>
                      </div>
                    ))}
