...
const PropertyDetails: React.FC<PropertyProps> = ({ properties }) => {
  ...
  return (
    <>
      ...
      {/* Property Location */}
      <div>
        <p className="text-default-500 text-sm">{properties.location}</p>{" "}
      </div>
      ...
      <div className="flex flex-col gap-6 mt-4">
        ...
        <div>
          ...
          <p className="text-default-500 text-md">{properties.description}</p>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          {properties.status === "For Sale" ? (
            <>
              <div className="flex items-center gap-2 bg-default-200 px-2 py-2 rounded-lg">
                ...
                <p className="text-base text-violet-600">{properties.status}</p>
              </div>
              ...
              {properties.sale_type === "RFO" ? (
                ...
                <>
                  ...
                  <div className="flex items-center gap-2 bg-default-200 px-2 py-2 rounded-lg">
                    ...
                    <p className="text-base text-violet-600">
                      {new Date(properties.turnover).toLocaleDateString(
                        "en-US",
                        {
                          weekday: "long",
                          year: "numeric",
          ...
          <div className="flex items-center gap-2 bg-default-200 px-2 py-2 rounded-lg">
            ...
            <p className="text-base text-violet-600">{properties.area} sqm</p>
          </div>
        ...
        <div>
          ...
          <div className="inline-flex flex-wrap gap-2 mt-4">
            {amenities.map((amenity, index) => (
              <div
                key={index}
                className="bg-gray-100 px-2 py-1 rounded-md text-sm hover:bg-gray-200 "
              >
                {amenity}
              </div>
            ))}
        ...
        <div className="w-full py-12">
          ...
          <Nearby currentPropertyId={properties.id} properties={[properties]} />
        </div>
        <FAQ />
      </div>
    </>
